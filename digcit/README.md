...
**18 activities, all free and open:** every grade K–8 has one **featured** lesson
plus one **full** per-grade breakout — both fully playable on the public site, no
login, nothing collected.

**Live:** https://mglearn.github.io/activities/digcit/ — all lessons + the
[🕹️ Critical Thinking Arcade](https://mglearn.github.io/activities/digcit/arcade/) (three free,
no-login sorting games, one per grade band, in 7 languages).

Sibling suite: [Gen AI Literacy Breakouts](https://mglearn.github.io/activities/genailit/)
(same engine, AI-adjacent TEKS strands).

## The activities

Each grade has a featured lesson and a full lesson — both free and open (TEKS
...
grade68/           band (Grades 6–8) — same shape
...
hosting/           nginx config (+ optional Node auth gateway, unused while open)
...
digcit is **fully open** — the whole suite is static and can be served by anything
(GitHub Pages, nginx, or embedded in LearnDash; see
[`docs/learndash-deployment-plan.md`](docs/learndash-deployment-plan.md)). Build the
public tree with `node scripts/build-pages.js` (outputs `_site/`). The optional
session-auth Node gateway in [`hosting/`](hosting/README.md) is retained for a
possible future licensed tier but is **not used** while the suite is open.
