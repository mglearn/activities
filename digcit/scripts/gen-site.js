/* gen-site.js — generate all non-activity pages of the Digital Citizenship suite
   ...
   the site stays consistent as content changes. Produces:
     index.html                         suite landing (featured + full lessons, all open)
...
const PALETTE = ':root{--navy:#0a2e63;--navy-d:#061f45;--red:#c1121f;--red-d:#8b0d16;--gold:#f5b800;--gold-d:#c08e00;--paper:#f4f8fd;--ink:#14203a;--ink-soft:#4b5a78;--card:#fff;--line:#dbe6f5;--good:#2f9e44;--bad:#e03131;--c1:#0a2e63;--c2:#2f6fe0;--c3:#16697a;--c4:#f5b800;--c5:#0d47a1;--bg-a:rgba(245,184,0,.12);--bg-b:rgba(10,46,99,.10)}';
...
// the full per-grade lesson (open); the FEATURED lesson per grade is a separate -free
...
const CLEAR = [
  ...
  { L: 'L', k: 'Lens', color: '#0e7490', q: 'What assumptions am I bringing?', pts: ['What do I already believe?', 'Am I reacting to evidence or past experience?'] },
  { L: 'E', k: 'Evidence', color: '#1f7a34', q: 'What supports the claim?', pts: ['Strong: facts, data, direct observations', 'Weak: rumors, impressions', 'Missing: what do I still need to know?'] },
  { L: 'A', k: 'Alternatives', color: '#b45309', q: 'What else could explain this?', pts: ['Consider other causes.', 'Consider other viewpoints.', 'Avoid oversimplifying.'] },
...
function teacherNav(depth) {
  ...
  <summary><span aria-hidden="true">👩‍🏫</span> Teachers <span aria-hidden="true">▾</span></summary>
  <div class="tm-panel">
...
<div class="wrap">
<nav class="topnav" aria-label="Site tools">${langbar}${teacherNav(depth)}</nav>
<main id="main">
${body}
</main>
${scripts}</div>
...
function card(grade, { depthToBand }) {
  ...
  // Band-hub card → teacher launch page (premise + standards, then the activity).
  return `<a class="card" href="${teacher}">
    ...
    <span class="tier"><span class="freeflag">FREE</span></span>
    <span class="cgo" data-i18n="card.startPaid">View the lesson →</span>
  </a>`;
}

// Activity-object card (used on the landing and library). kind: 'free' | 'full'
// — kept only as a descriptor; both are open and link to the playable page.
// Optional hero banner image (Canva-generated). Emitted only if the PNG exists
// in assets/, so pages degrade gracefully before an image is produced.
function heroImg(file, depth) {
  if (!fs.existsSync(path.join(ROOT, 'assets', file))) return '';
  return `<img class="hero-img" src="${depth ? '../' : ''}assets/${file}" alt="" width="1600" height="900" loading="eager">\n    `;
}
function actCard(a, kind, depth) {
  const locked = false;  // digcit is fully open — every lesson is playable
...
function suiteLanding() {
  ...
  const body = `  <div class="hero">
    ${heroImg('hero-digcit.png', 0)}<div class="eyebrow" data-i18n="hero.eyebrow">${esc(si('hero.eyebrow'))}</div>
  ...
  ${E('h2', 'The full per-grade lessons')}
  ${E('p', 'The complete lesson for each grade — free and open, just like the featured set. Jump straight in, or preview the premise and standards on each teacher page.', 'class="section-note"')}
  <div class="cards">
    ${paidCards}
  </div>
...
function bandHub(band, grades, opts) {
  ...
  const paidNote = opts.paid ? EH('div', '<strong>Free &amp; open.</strong> Every lesson in this band is free to play — no login, nothing collected. Each card opens the teacher page (premise + standards), then the activity itself.', 'class="panel tip"') : '';
  const body = `  <div class="crumb" role="navigation" aria-label="Breadcrumb">${E('a', '‹ Suite home', 'href="../index.html"')}</div>
  <div class="hero">
    ${heroImg('hero-' + band + '.png', 1)}${E('div', opts.label, 'class="eyebrow' + (opts.paid ? ' gold' : '') + '"')}
    ${E('h1', opts.title)}
...
function teacherPage(a, focus) {
  ...
  const body = `  <div class="crumb" role="navigation" aria-label="Breadcrumb">${E('a', '‹ ' + BAND_NAME[a.band] + ' hub', 'href="index.html"')} · ${E('a', 'Suite home', 'href="../index.html"')}</div>
  <div class="hero">
    ${heroImg('hero-grade' + grade + '.png', 1)}<div class="eyebrow${a.tier === 'paid' ? ' gold' : ''}">${S('Teacher launch')} · ${S('Grade')} ${grade} · TEKS ${a.teks}</div>
    ${E('h1', U['header.h1'])}
    ${E('p', U['header.sub'], 'class="lede"')}
    <div class="btnrow">
      ${E('a', 'Open activity', 'class="btn" href="' + a.id + '-student.html"')}
...
function correlation() {
  function bandTable(label, grades) {
    const rows = grades.map(g => {
      ...
      return `<tr>
        <td><span class="lk">${S(U['header.h1'])}</span><br><span class="small">${S('Grade')} ${g} · ${S(a.tier === 'paid' ? 'full' : 'free')}</span></td>
        <td>${lockList}</td>
...
function guide() {
  ...
  <img class="feature-img" src="assets/ctobs-feature.png" alt="Build better online judgment with Digital Citizenship CTOBs — the CLEAR steps: Claim, Lens, Evidence, Alternatives, Response. Think before you click." width="1672" height="941" loading="eager">
  ...
  <img class="feature-img" src="assets/ctobs-why-teachers.png" alt="Why teachers love CTOBs: critical thinking (students weigh evidence; one clue is true but irrelevant; discussion replaces guessing), teacher-friendly (no logins; works whole-group, in partners, and in stations), accessibility and multilingual support, and curriculum support (pairs with TEKS, UDL, and ELPS). CLEAR makes learning visible: Claim, Lens, Evidence, Alternatives, Response." width="1672" height="941" loading="lazy">
  ...
  <img class="feature-img" src="assets/ctobs-clear-framework.png" alt="Digital Citizenship CTOBs and the CLEAR framework. What is a CTOB: short clue-based investigations; students solve four locks using evidence; one clue is true but irrelevant; no logins and no data collection. CLEAR in action: 1 Claim (what is being said?), 2 Lens (what assumptions am I bringing?), 3 Evidence (what supports the claim?), 4 Alternatives (what else could explain this?), 5 Response (what should I do now?). Why it matters: builds safer online choices, strengthens reasoning not guessing, active learning, discussion and reflection." width="1672" height="941" loading="lazy">

  ${E('h2', 'How to deploy')}
  <div class="panel">${EH('ul', deploy)}</div>

  ${E('h2', 'Two lessons per grade')}
  <div class="panel gold">${EH('p', '<strong>Free featured lessons — one per grade.</strong> Each grade has a fully-playable featured breakout on the home page. Use them with anyone, anywhere — no login, nothing collected.')}${EH('p', '<strong>The full per-grade lessons.</strong> The complete lesson for each grade (Grades K–8) is also free and open — same engine, same no-login, nothing-collected design. Every teacher page and this guide are open too.')}</div>
  <img class="feature-img" src="assets/ctobs-across-grades.png" alt="Digital Citizenship CTOBs across the grades, using CLEAR to think through online choices. K–2: kind online behavior, simple passwords, digital footprints. Grades 3–5: group chats, private information, responsible sharing. Grades 6–8: phishing, clickbait, copyright, source credibility. CLEAR reflection routine: Claim, Lens, Evidence, Alternatives, Response." width="1672" height="941" loading="lazy">
...
function implementation(band, grades, opts) {
  ...
  const body = `  <div class="crumb" role="navigation" aria-label="Breadcrumb">${E('a', '‹ ' + opts.hub, 'href="index.html"')} · ${E('a', 'Suite home', 'href="../index.html"')}</div>
  <div class="hero">
...
function policy(band, label) {
  const body = `  <div class="crumb" role="navigation" aria-label="Breadcrumb">${E('a', '‹ ' + label + ' hub', 'href="index.html"')} · ${E('a', 'Suite home', 'href="../index.html"')}</div>
  <div class="hero">
...
function library() {
  ...
  const body = `  <div class="crumb" role="navigation" aria-label="Breadcrumb"><a href="index.html">‹ Suite home</a></div>
  <div class="hero">
  ...
  <div class="panel">
    ...
    <div class="filters">
      ...
      <span class="fgroup"><span class="flabel">Tier</span>
        ...
        <button class="chip" data-tier="paid">Full lessons</button></span>
...
(function(){
  ...
  function render(){
    ...
    grid.innerHTML = list.map(function(a){
      return '<a class="card" href="'+a.href+'">'
        ...
        + '<span class="tier"><span class="freeflag">FREE</span></span>'
        + '<span class="cgo">Start the breakout →</span></a>';
    }).join('');
...
function scope() {
  const rows = GRADES.map(g => {
    ...
    return `<tr>
        <td><span class="lk">${S('Grade')} ${g}</span><br><span class="small">${S(a.tier === 'paid' ? 'full' : 'free')} · ${a.teks}</span></td>
...
function lessons() {
  ...
  const examples = GRADES.map(g => {
    ...
    ${EH('h3', 'Grade ' + g + ' — ' + esc(U['header.h1']) + ' <span class="small">(' + a.teks + ' · ' + (a.tier === 'paid' ? 'full' : 'free') + ')</span>')}
...
function udl() {
  ...
  const body = `  <div class="crumb" role="navigation" aria-label="Breadcrumb">${E('a', '‹ Suite home', 'href="index.html"')} · ${E('a', 'Teacher guide', 'href="guide.html"')}</div>
  <div class="hero">
...
suiteLanding();
library();
bandHub('gradek2', ['K', '1', '2'], { paid: true, label: 'Grades K–2', title: 'Grades K–2', lede: 'The full early-elementary lessons — trusted grown-ups, kind words, private information, passwords, and your first digital trail. Free and open — each grade also has a free featured lesson on the home page.' });
bandHub('grade35', [3, 4, 5], { paid: true, label: 'Grades 3–5', title: 'Grades 3–5', lede: 'The full elementary lessons — digital footprints, kindness, credit, and staying safe. Free and open — each grade also has a free featured lesson on the home page.' });
bandHub('grade68', [6, 7, 8], { paid: true, label: 'Grades 6–8', title: 'Grades 6–8', lede: 'The full middle-school lessons — intellectual property, media literacy, cybersecurity, and source evaluation. Free and open — each grade also has a free featured lesson on the home page.' });
correlation();
guide();
scope();
lessons();
...
implementation('grade35', [3, 4, 5], { paid: false, hub: 'Grades 3–5', title: 'Grades 3–5 Implementation Plan', lede: 'Pacing, prerequisites, and extensions for the elementary band.', prereq: 'Students should be comfortable reading short paragraphs and clicking/tapping tiles.', extend: 'Connect to a class agreement on kindness and credit; revisit the footprint idea across the year.' });
implementation('grade68', [6, 7, 8], { paid: true, hub: 'Grades 6–8', title: 'Grades 6–8 Implementation Plan', lede: 'Pacing, prerequisites, and extensions for the middle-school band.', prereq: 'Students should be able to evaluate short arguments and are ready for real IP, security, and media-literacy vocabulary.', extend: 'Bridge to a media-literacy or intro-to-AI unit; have students audit a real (teacher-vetted) source using the Grade 8 criteria.' });
