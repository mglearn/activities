#!/usr/bin/env node
/* build-answer-key.js — regenerate the encrypted teacher answer key for ALL idiom
   breakouts, then embed the AES-256-GCM payload into answer-key.html. The password
   is NEVER stored here — pass it at run time:

     node activities/idioms/build-answer-key.js 'THE-PASSWORD'
...
// order & grouping from the manifest
const w = {}; eval(fs.readFileSync(path.join(ROOT, 'breakouts.js'), 'utf8').replace('window.IDIOMS_LIB', 'w.LIB'));
const LIB = w.LIB;

let body = `<style>
body{font-family:Nunito,system-ui,sans-serif;max-width:860px;margin:20px auto;padding:0 18px;color:#122a2e}
h1{color:#0b5563}
h2{color:#0e7490;border-bottom:2px solid #f59e0b;padding-bottom:4px;margin-top:30px;font-size:1.15rem}
h3{color:#0891b2;margin:20px 0 6px;font-size:1.02rem}
.ak{background:#f2fbfc;border-left:4px solid #0e7490;padding:10px 14px;margin:9px 0;border-radius:6px;font-size:.92rem;line-height:1.5}
em{color:#4b6a70}
</style>
<h1>Answer Key — Idioms &amp; Sayings for Multilingual Learners</h1>
<p>Teacher use only. Every lock is answerable from the clue set; the rationale models the thinking students should be able to articulate. Covers all ${LIB.length} breakouts.</p>`;
