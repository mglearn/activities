...
**Live now** (`activities/idioms/`, all deployed):
...
**Build commands:** `node activities/idioms/build.js` (breakouts + manifest) · `node activities/idioms/build-answer-key.js '<passcode>'` (answer key) · `cd breakouts && node build-catalog.js` (site catalog).
...
1. Suite lives in `activities/idioms/`, reusing `../assets/i18n.js` + `../assets/breakout.js` and a
