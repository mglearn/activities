...
const PALETTE = ':root{--navy:#4338ca;--navy-d:#312e81;--red:#c1121f;--red-d:#8b0d16;--gold:#06b6d4;--gold-d:#0e7490;--paper:#f5f6ff;--ink:#14203a;--ink-soft:#4b5a78;--card:#fff;--line:#e0e7ff;--good:#2f9e44;--bad:#e03131;--c1:#4338ca;--c2:#7c3aed;--c3:#0e7490;--c4:#06b6d4;--c5:#6d28d9;--bg-a:rgba(6,182,212,.12);--bg-b:rgba(67,56,202,.10)}';
...
function actCard(a, kind, depth) {
  const locked = false;  // genailit is fully open — every lesson is playable
...
function suiteLanding() {
  ...
  const body = `  <div class="hero">
    ${heroImg('hero-genailit.png', 0)}<div class="eyebrow" data-i18n="hero.eyebrow">${esc(si('hero.eyebrow'))}</div>
...
suiteLanding();
library();
bandHub('gradek2', ['K', '1', '2'], { paid: true, label: 'Grades K–2', title: 'Grades K–2', lede: 'The full early-elementary lessons — what an AI helper is, how it makes guesses, checking its answers, and using it safely with a grown-up. Free and open — each grade also has a free featured lesson on the home page.' });
bandHub('grade35', [3, 4, 5], { paid: true, label: 'Grades 3–5', title: 'Grades 3–5', lede: 'The full elementary lessons — what AI is, how it learns from data, and how to prompt it and check its work. Free and open — each grade also has a free featured lesson on the home page.' });
bandHub('grade68', [6, 7, 8], { paid: true, label: 'Grades 6–8', title: 'Grades 6–8', lede: 'The full middle-school lessons — spotting AI-generated media, tracing where bias comes from, and creating with AI responsibly. Free and open — each grade also has a free featured lesson on the home page.' });
correlation();
guide();
scope();
lessons();
...
implementation('grade35', [3, 4, 5], { paid: false, hub: 'Grades 3–5', title: 'Grades 3–5 Implementation Plan', lede: 'Pacing, prerequisites, and extensions for the elementary band.', prereq: 'Students should be comfortable reading short paragraphs and clicking/tapping tiles.', extend: 'Connect to a class agreement on checking facts and giving credit; revisit the "AI guesses from patterns" idea across the year.' });
implementation('grade68', [6, 7, 8], { paid: true, hub: 'Grades 6–8', title: 'Grades 6–8 Implementation Plan', lede: 'Pacing, prerequisites, and extensions for the middle-school band.', prereq: 'Students should be able to evaluate short arguments and are ready for real vocabulary: training data, bias, hallucination, disclosure.', extend: 'Bridge to a media-literacy or intro-to-CS unit; have students evaluate a teacher-vetted example of AI output using the Grade 6–8 criteria.' });
